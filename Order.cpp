...
//Public Methods
    ...
    //User Interaction
        void Order::DisplayOrderAsTicket(){//*Clean this up later with iterative spacing, rather than triple if statements, Ticket resizes depending on near side-part or near rack
        ...
        void Order::TicketDraggingBehavior(){//*Drags the ticket around by the mouse. Kind of a bad name because it also displays the ticket, but idk if there's an easy way, to separate the dragging behavior and the display behavior without it being a bit confusing. 
    ...
    //Accsessors
        ...
        int Order::GetSauseID(){
    return sauceID;
}

        int Order::GetToppingID(int index){
            return toppingID[index];
        }

        string Order::GetToppingName(int index){
            return TranslateTopping(toppingID[index]);
        }

        int Order::GetToppingAmount(int index){
            return toppingAmount[index];
        }

        int Order::GetCookTime(){
            return cookTime;
        }

        int Order::GetSliceAmount(){
    return sliceAmount;
}
        ...
        bool Order::GetActiveStatus(){
            return isActive;
        }
    
        void Order::SetActive(){
            isActive = true;
        }

        void Order::SetInactive(){
            isActive = false;
        }

        void Order::SetX(int newX){
            x = newX;
        }

        void Order::SetY(int newY){
            y = newY;
        }
        //Misc
